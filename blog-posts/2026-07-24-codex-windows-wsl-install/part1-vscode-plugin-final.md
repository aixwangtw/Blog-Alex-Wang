...
tags: [Codex, VS Code, 安裝教學]
faqs:
  - question: 我最後到底要安裝哪些工具？
    answer: 這一篇只需要兩個：VS Code 編輯器與 OpenAI 官方 Codex 插件，裝完就能開始用。Codex CLI 與 WSL2 是進階安裝，想在終端機操作或接自動化流程時，再看下集即可。
  - question: 插件出現 Unable to send message 怎麼辦？
    answer: 如果完整訊息是 Add a project to use ChatGPT，代表 VS Code 目前是空白視窗。先從 File 選單按 Open Folder 開啟或建立一個專案資料夾，再回到 Codex 重新送出訊息。
  - question: 安裝 Codex 插件後，就能在終端機輸入 codex 嗎？
    answer: 不能。插件與 CLI 是兩個不同的安裝項目，插件只在 VS Code 視窗裡使用。想在終端機輸入 codex 操作，請看下集完成 Codex CLI 的安裝。
  - question: 為什麼要安裝 CLI？VS Code 插件不夠嗎？
    answer: 日常讓 AI 讀專案、改檔案、檢查差異，插件版就夠用。CLI 適合安裝工具、連續執行指令、測試與排錯，以及接自動化流程；確定有這些需求再看下集安裝即可，兩者不是誰比較高級。
  - question: 為什麼不直接安裝桌面版？不是比較簡單嗎？
    answer: 桌面版適合聊天與管理多個工作；VS Code 可以直接開啟整個專案資料夾，左側看檔案、中間改內容、右側請 Codex 協助，還內建 Git 版本紀錄。兩者不衝突，需要在編輯器裡改專案時用 VS Code 插件更順。
  - question: Codex 安裝失敗時，可以把這篇文章貼給 AI 嗎？
    answer: 可以。請同時貼上文章連結、畫面上的完整錯誤訊息，以及你卡住的步驟。不要只說安裝失敗，保留原文才能讓 AI 對照本文環境與步驟排查。
---
...
- 你想在**終端機**直接下指令操作 Codex，不開 VS Code 也能用
- 你想把 Codex 接進**自動化流程**（排程、腳本、CI）
...
安裝過程卡住了？加 LINE 把錯誤訊息貼給我，簡單問題**免費**幫你看。
