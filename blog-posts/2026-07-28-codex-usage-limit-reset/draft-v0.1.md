---
title: Codex 額度燒完怎麼辦？省 Token 與 Full reset 手動重置教學
description: Codex 額度燒完時，先到使用量頁面查看並兌換 Full reset 重置券；日常再靠切換 Luna、縮小任務範圍與開新 session，讓額度別燒這麼快。
slug: codex-usage-limit-reset
status: draft
featured: false
pubDate: 2026-07-28
tags: [ChatGPT, Codex, Token, 使用額度]
testedDate: 2026-07-28
testedEnvironment: ChatGPT Windows 桌面版與 Codex CLI
faqs:
  - question: Full reset 會重置一般 ChatGPT 對話的 GPT 額度嗎？
    answer: 目前沒有官方公開文件說明 Full reset 會重置一般 ChatGPT 對話的模型用量，因此本文只把它視為 Codex 使用量重置；官方明確說明 ChatGPT Work 與 Codex 共用用量，其他共用範圍仍以帳號頁面的即時說明為準。
  - question: Codex 額度用完後只能等系統重置嗎？
    answer: 不一定。除了等待週期重置，Plus 與 Pro 使用者可視帳號資格購買額外 credits；若帳號內有 Full reset，也能在使用量頁面或 Codex CLI 的 /usage 選單手動兌換。
  - question: 使用量頁面看不到 Full reset 重置券怎麼辦？
    answer: 重置券在 ChatGPT 網頁版或桌面版的設定→使用量就看得到，也可以直接開 chatgpt.com/codex/settings/usage，兩邊是同一份資料。都沒看到，通常代表帳號目前沒有可用重置券，可等下一波發放。
  - question: 每完成一個任務都要開新 session 嗎？
    answer: 不同目標建議用 /new 開新 session，避免舊對話持續佔用上下文。同一個長任務仍需保留決策與進度時，則用 /compact 摘要舊內容，不必從頭重講。
  - question: Luna、GPT-5.5 與 Sol 應該怎麼選？
    answer: 日常整理、固定格式修改與大量輕量工作優先用 Luna；既有流程在 GPT-5.5 已穩定時也可繼續使用。只有架構、安全性、模糊除錯或重大決策等需要深入判斷的任務，再切到 Sol。
  - question: 把規則寫進 AGENTS.md 一定比較省額度嗎？
    answer: 只有經常重複且保持精簡的規則適合放進 AGENTS.md，例如不要自動提交、修改後要跑哪些檢查。Codex 每次啟動 session 都會讀取這些指示，若把完整文件與大量背景全部塞進去，反而會增加上下文與額度消耗。
---
